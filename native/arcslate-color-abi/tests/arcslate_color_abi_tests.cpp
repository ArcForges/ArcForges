...
int main()
{
    uint32_t major = 0;
    uint32_t minor = 0;
    if (arc_color_get_abi_version(&major, &minor) != ARC_OK || major != 1 || minor != 0) {
        return 1;
    }
    arc_mut_buffer_t query{};
    if (arc_color_get_build_info(&query) != ARC_BUFFER_TOO_SMALL || query.required == 0) {
        return 2;
    }
    ...
    if (arc_color_get_build_info(&output) != ARC_OK || output.required != value.size()) {
        return 3;
    }
    if (arc_color_get_abi_version(nullptr, &minor) != ARC_INVALID_ARGUMENT) {
        return null_output_failure;
    }
    arc_error_info_t error{.struct_size = sizeof(arc_error_info_t), .struct_version = 1};
    if (arc_color_get_last_error(&error) != ARC_BUFFER_TOO_SMALL || error.status != ARC_INVALID_ARGUMENT) {
