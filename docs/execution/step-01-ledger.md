...
| 6 | CI referable by stable job name | **Met** — Step 01.06 originally satisfied the ten-name contract. The later authorized topology change removed only the redundant `build` gate; current `pr-gate` declares the nine retained managed names, `runtime-publish-smoke` exists, and `release-train` declares all seven `train-*` jobs, all pinned by `MandatedCiJobNamesArePresent`; every gate's command was executed locally, and the three failures reported on the first run were fixed |
